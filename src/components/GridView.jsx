...
// ── Domain row order — Entra ID bands ─────────────────────────────────────────
const ROW_DEFS = [
  { cats: ['core'],                       label: 'CORE IDENTITY',          id: 'core' },
  { cats: ['access'],                     label: 'ACCESS & AUTH',          id: 'access' },
  { cats: ['governance','protection'],    label: 'GOVERNANCE & PROTECTION',id: 'governance' },
  { cats: ['workload','external'],        label: 'WORKLOAD & EXTERNAL',    id: 'workload' },
  { cats: ['network','ecosystem'],        label: 'NETWORK & ECOSYSTEM',    id: 'network' }
]

// ── Atomic symbols (2-letter abbreviations) ──────────────────────────────────
const SYMBOLS = {
  'users': 'Us', 'groups': 'Gr', 'admin-units': 'Au', 'custom-attributes': 'Ca',
  'conditional-access': 'CA', 'mfa': 'Mf', 'auth-methods': 'Am', 'passkeys': 'Pk',
  'auth-strengths': 'As', 'cae': 'Ce', 'token-protection': 'Tp', 'named-locations': 'Nl',
  'pim': 'Pi', 'entitlement-mgmt': 'Em', 'access-reviews': 'Ar',
  'lifecycle-workflows': 'Lw', 'cross-tenant-sync': 'Cs',
  'identity-protection': 'Ip', 'sign-in-logs': 'Sl', 'risk-policies': 'Rp',
  'app-registrations': 'Ap', 'service-principals': 'Sp',
  'managed-identities': 'Mi', 'workload-id-premium': 'Wp',
  'b2b': 'B2', 'external-id': 'Ei', 'verified-id': 'Vi',
  'cross-tenant-access': 'Ct', 'entra-connect': 'Ec',
  'global-secure-access': 'Ga', 'internet-access': 'Ia', 'private-access': 'Pa',
  'intune': 'In', 'defender-identity': 'Di', 'purview': 'Pv', 'sentinel': 'Sn'
}
      ...
      <div style={{ padding: '20px 24px 48px' }}>
        ...
        <svg style={{
          ...
          {EDGES.map((e, i) => {
            ...
            if (!isSelEdge) {
              if (sourceCat === 'access') catDashExtra = '4 4'
              else if (sourceCat === 'protection') { catDashExtra = undefined; catStrokeExtra = strokeW * 1.2 }
              else if (sourceCat === 'governance') catDashExtra = '8 3'
              else if (sourceCat === 'core') catDashExtra = '2 3'
            }
