...
// Standalone 5th view of the Purview Map app. Scene-by-scene narrative reveals.
    ...
    return (
      <g transform={`translate(${n.x},${n.y})`}
        ...
        <g style={{ transform:`scale(${scale})`, transformOrigin:`${n.x}px ${n.y}px` }}>
          ...
          <text x="-46" y="-2" fontSize="11" fill={SV.ink} fontWeight="600">{n.name.length > 18 ? n.name.slice(0, 17) + '…' : n.name}</text>
          <text x="-46" y="11" fontSize="8.5" fill={c.ring} fontWeight="600" letterSpacing="0.04em">{c.label.toUpperCase()}</text>
        </g>
      </g>
    )
  }
  ...
  return (
    <div style={{display:'flex', alignItems:'center', gap:12, padding:'10px 18px', borderBottom:`1px solid ${SV.divider}`, background:'#fff'}}>
      <div style={{display:'flex', alignItems:'center', gap:8}}>
        <div style={{width:26, height:26, background:'#C5377A', borderRadius:6, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:11, fontWeight:700}}>E</div>
        <div style={{fontSize:13, fontWeight:600, letterSpacing:'-0.01em'}}>Entra ID Map</div>
      </div>
